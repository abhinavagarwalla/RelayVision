...
def write_data(img_data_path):
    ...
    categories.sort()
    addrs_temp = [glob.glob(img_data_path + i + '/*') for i in categories]
    index = np.array([len(addrs_temp[i])-1 for i in range(len(categories))])
    addrs = []
    ...
    if split_str == "train":
        ...
        while index.any() > 0:
            for j in range(len(categories)):
                if index[j] > 0:
                    if categories[j] == "other":
                        for iterrun in range(4):
                            addrs.append(addrs_temp[j][index[j]])
                            if index[j] == 0:
                                break
                            else:
                                index[j] -= 1
...
if __name__=="__main__":
    home = expanduser("~")
    img_data_path = home + '/abhinav/RelayVision/data/' + split_str + os.sep # for Aggie's laptop    
